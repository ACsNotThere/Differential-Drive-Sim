...
public class ArmSubsystem extends SubsystemBase {
  /* Pseudo code for ArmSubsystem
   * Declare feedforward object with arm constants in parameters
   * Declare motor + simulation and visualization variables & objects
   * 
   * Constructor will initialize motor variables
   * Will also use getters and setters for variables
   * Getters return signal values
   * Setters do not return values
   * Motor parameters and current limits are configured by a config object
   * 
   * Create armSim object with parameters
   * 
   * Period method to update SmartDashboard
   * Calls on methods to refresh and replace numbers
   * 
   * simulationPeriodic method to simulate
   * Will also update the SmartDashboard's information by calling methods
   * Other methods (getters and setters) called will set and update the arm's velocity and position
   * 
   * Declare Command methods that use lambda expressions with setters
   */
  // Feedforward
  ...
  private final TalonFX motor  = new TalonFX(ArmConstants.krakenCanID);
  private final CANcoder absoluteEncoder = new CANcoder(ArmConstants.canIDCanCoder);
  ...
  private final StatusSignal<AngularVelocity> vSignal;
  ...
  public ArmSubsystem() {
    ...
    vRequest = new VelocityVoltage(0).withSlot(0);
    posSignal = motor.getPosition();
    absPosSignal = absoluteEncoder.getAbsolutePosition();
    vSignal = motor.getVelocity();
    ...
    Slot0Configs slot0 = krakenConfig.Slot0;
    slot0.kP = ArmConstants.kP;
    slot0.kI = ArmConstants.kI;
    slot0.kD = ArmConstants.kD;    

    CurrentLimitsConfigs currentLimits = krakenConfig.CurrentLimits;
    ...
    krakenConfig.Feedback.SensorToMechanismRatio = ArmConstants.gearRatio;
    krakenConfig.MotionMagic.MotionMagicCruiseVelocity = 10.0;
    krakenConfig.MotionMagic.MotionMagicAcceleration = 20.0;

    krakenConfig.MotorOutput.NeutralMode = ArmConstants.brakeMode ? NeutralModeValue.Brake : NeutralModeValue.Coast; // Set brake mode
    ...
    motor.setPosition(getAbsolutePositionDouble());
  ...
  public void periodic() {
    ...
    BaseStatusSignal.refreshAll(posSignal, vSignal, voltageSignal, statorCurrentSignal, temperatureSignal);
  ...
  // Gets absolute position in rotations
  public double getAbsolutePositionDouble() {
    return absPosSignal.getValueAsDouble();
  }


  // Method to get current velocity of the arm (in rotations per second)
  public double getVelocity() {
    return vSignal.getValueAsDouble();
  }
  ...
  public void setVelocity(double velocityDegPerSec, double acceleration) {
    ...
    motor.setControl(vRequest.withVelocity(velocityRotations).withFeedForward(ffVolts));
  }
